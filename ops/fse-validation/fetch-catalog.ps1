...
$entries = $tree.tree | Where-Object { $_.type -eq 'blob' -and ($_.path -like 'schema/POCD_MT000040UV02/*.xsd' -or $_.path -eq 'schematron/schematron_RSA_v8.3.sch') }
if (@($entries).Count -ne 12) { throw 'Unexpected official catalog file count.' }
$hashes = [ordered]@{}
...
Write-Output "Downloaded 12 official files at $revision into $destinationPath"
