...
class AddSharedAgendaPatientsFeature extends Migration
{
    protected $DBGroup = 'platform';

    private const FEATURE_KEY = 'shared_agenda_patients';
    ...
    public function up()
    {
        ...
        if (!$feature) {
            $payload = [
                ...
                'feature_name' => 'Pazienti condivisi tra dottori',
                'feature_scope' => 'workflow',
                'description' => 'Il tenant master decide se i dottori devono vedere solo i propri pazienti agenda oppure un archivio condiviso tra tutti i dottori dello spazio.',
                'default_enabled' => 0,
                'created_at' => $now,
                'updated_at' => $now,
            ];
            ...
            if ($this->db->fieldExists('icon_class', 'platform_features')) {
                $payload['icon_class'] = 'fa-address-book';
            }
            ...
            if ($this->db->fieldExists('icon_class', 'platform_features')) {
                $update['icon_class'] = 'fa-address-book';
            }
