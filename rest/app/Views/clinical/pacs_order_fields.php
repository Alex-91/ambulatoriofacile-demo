<?php $v=$v ?? []; ?>
<div class="grid">
<label class="full">Prestazione richiesta<input name="description" <?= !empty($v['appointment_id']) ? 'readonly' : '' ?> maxlength="64" required value="<?= esc($v['description'] ?? '','attr') ?>"></label>
...
<label>Data e ora previste<input type="datetime-local" name="scheduled_at" <?= !empty($v['appointment_id']) ? 'readonly' : '' ?> required value="<?= esc($v['scheduled_at'] ?? '','attr') ?>"><small>Fuso orario Italia.</small></label>
