...
if ([string]::IsNullOrWhiteSpace($EnvFile)) {
    $EnvFile = Join-Path $restRoot '.env'
}

$logDirectory = Join-Path $restRoot 'writable\ts_preview'
$metaLogPath = Join-Path $logDirectory "ts_test_preview_${Port}.log"
$stdoutLogPath = Join-Path $logDirectory "ts_test_preview_${Port}.stdout.log"
$stderrLogPath = Join-Path $logDirectory "ts_test_preview_${Port}.stderr.log"
...
if (-not $phpCandidates) {
    throw 'php.exe non trovato per la preview TS.'
}
...
if ($existingPid -gt 0) {
    ...
    if ($running) {
        "[$(Get-Date -Format s)] TS test preview gia attiva su $previewUrl con pid $existingPid" | Out-File -FilePath $metaLogPath -Encoding utf8 -Append
        Write-Output "TS test preview gia attiva su $previewUrl"
        exit 0
    }
}
