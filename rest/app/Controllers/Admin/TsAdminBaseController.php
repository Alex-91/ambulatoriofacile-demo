    ...
    protected function ensureAccess()
    {
        ...
        if (!$isPlatformAdminSession && !$hasTenantOperationalAccess) {
            return redirect()->to(site_url('/'))
                ->with('error', 'La console operativa TS e disponibile solo per admin piattaforma o responsabili dello spazio.');
        }

        if ((int) ($tenantScope['tenant_id'] ?? 0) <= 0) {
            $fallbackUrl = $hasTenantOperationalAccess ? portal_tenant_space_url('fatturazione-ts') : site_url('admin');
        ...
        if (empty($tenantScope['feature_enabled'])) {
            ...
            return redirect()->to($fallbackUrl)->with('error', 'Fatturazione TS non attiva per questo spazio.');
        }
