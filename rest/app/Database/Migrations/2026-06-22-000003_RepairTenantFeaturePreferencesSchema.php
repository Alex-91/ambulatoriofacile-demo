...
class RepairTenantFeaturePreferencesSchema extends Migration
{
    ...
    public function up()
    {
        ...
        $this->ensureTenantFeaturePreferenceTable();
        $this->ensurePreferenceConfigColumn();
        $this->backfillSeedFeatureMetadata();
    }

    public function down()
    {
        // Repair migration intentionally left as no-op to avoid destructive rollbacks on live data.
    }

    private function ensureFeatureCatalogColumns(): void
    {
    ...
    private function ensureTenantFeaturePreferenceTable(): void
    {
        ...
        if (
            !$this->db->tableExists('platform_tenants')
            || !$this->db->tableExists('platform_features')
            || !$this->db->tableExists('platform_users')
        ) {
            return;
        }
